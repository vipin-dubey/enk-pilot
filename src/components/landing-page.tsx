        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-16 lg:mb-24">
            <h2 className="text-3xl lg:text-5xl font-black font-outfit tracking-tighter text-slate-900 mb-6">
              {t.rich('howItWorks.title', { highlight: (chunks) => <span className="text-blue-600">{chunks}</span> })}
            </h2>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-20">
            <h2 className="text-4xl lg:text-5xl font-black font-outfit tracking-tighter text-slate-900 mb-6">
              {t.rich('features.title', { highlight: (chunks) => <span className="text-blue-600 bg-clip-text uppercase">{chunks}</span> })}
            </h2>
            <p className="text-slate-600 font-medium max-w-2xl mx-auto text-base lg:text-lg leading-relaxed px-4">
              {t.rich('features.description', { highlight: (chunks) => <span className="text-slate-900 font-bold">{chunks}</span> })}
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <FeatureCard icon={<ShieldCheck className="h-6 w-6 text-blue-600" />} title={t('features.safeToSpend.title')} description={t('features.safeToSpend.desc')} gradient="from-blue-500/10 to-transparent" />
            <FeatureCard icon={<Sparkles className="h-6 w-6 text-indigo-600" />} title={t('features.aiScan.title')} description={t('features.aiScan.desc')} gradient="from-indigo-500/10 to-transparent" />
            <FeatureCard icon={<Activity className="h-6 w-6 text-emerald-600" />} title={t('features.pulse.title')} description={t('features.pulse.desc')} gradient="from-emerald-500/10 to-transparent" />
            <FeatureCard icon={<Target className="h-6 w-6 text-amber-600" />} title={t('features.deadline.title')} description={t('features.deadline.desc')} gradient="from-amber-500/10 to-transparent" />
            <FeatureCard icon={<BarChart3 className="h-6 w-6 text-cyan-600" />} title={t('features.analysis.title')} description={t('features.analysis.desc')} gradient="from-cyan-500/10 to-transparent" />
            <FeatureCard icon={<Globe className="h-6 w-6 text-purple-600" />} title={t('features.currency.title')} description={t('features.currency.desc')} gradient="from-purple-500/10 to-transparent" />
            <FeatureCard icon={<Download className="h-6 w-6 text-slate-600" />} title={t('features.exports.title')} description={t('features.exports.desc')} gradient="from-slate-500/10 to-transparent" />
            <FeatureCard icon={<Zap className="h-6 w-6 text-red-600" />} title={t('features.insights.title')} description={t('features.insights.desc')} gradient="from-red-500/10 to-transparent" />
          </div>
        </div>
      </section>
...
function FeatureCard({ icon, title, description, gradient }: { icon: React.ReactNode, title: string, description: string, gradient?: string }) {
  return (
    <div className="group relative p-8 bg-white rounded-[2rem] border border-slate-100 shadow-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10 overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="h-14 w-14 bg-slate-50 rounded-2xl flex items-center justify-center mb-8 border border-slate-100 group-hover:scale-110 group-hover:bg-white group-hover:shadow-lg transition-all duration-500">{icon}</div>
        <h3 className="text-xl font-black font-outfit text-slate-900 mb-3 tracking-tight group-hover:text-blue-600 transition-colors">{title}</h3>
        <p className="text-slate-600 font-medium leading-relaxed text-sm">{description}</p>
      </div>
      <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-x-4 group-hover:translate-x-0"><ArrowRight className="h-5 w-5 text-slate-300" /></div>
    </div>
  )
}
