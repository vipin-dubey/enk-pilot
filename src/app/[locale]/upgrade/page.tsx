...
import { Check, Sparkles, Globe, FileDown, ShieldCheck, Rocket, ArrowLeft, Receipt } from 'lucide-react'
              ...
              <div className="md:col-span-2 bg-slate-900 p-8 md:p-10 text-white flex flex-col justify-between">
                <div className="hidden md:block">
                  ...
                  <ul className="space-y-6">
                    {[
                      { icon: Receipt, text: locale === 'nb' ? 'AI Kvitteringsskanning' : 'AI Receipt Scan' },
                      { icon: Globe, text: locale === 'nb' ? 'Internasjonale valutaer' : 'International Currencies' },
                      { icon: FileDown, text: locale === 'nb' ? 'Ubegrenset eksport for regnskap' : 'Unlimited Accountant-Ready Export' },
                      { icon: ShieldCheck, text: locale === 'nb' ? 'AI Skatteinnikter' : 'AI Tax Insights', badge: locale === 'nb' ? 'lokal / privat' : 'local / private' },
                      { icon: ShieldCheck, text: locale === 'nb' ? 'MFA Autentisering' : 'MFA Authentication' },
                ...
                <div className="md:hidden">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { icon: Receipt, text: locale === 'nb' ? 'AI-Skan' : 'AI-Scan' },
                      { icon: Globe, text: locale === 'nb' ? 'Valuta' : 'Currency' },
                      { icon: FileDown, text: locale === 'nb' ? 'Ubegrenset eksport' : 'Unlimited Export' },
                      { icon: ShieldCheck, text: locale === 'nb' ? 'Skatte-AI' : 'Tax AI' },
                      { icon: ShieldCheck, text: 'MFA' },
              ...
              <div className="md:col-span-3 bg-white p-6 md:p-8 flex flex-col justify-center">
                ...
                <div className="space-y-6 md:space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {/* Monthly Option */}
                    <div className="group relative p-6 rounded-[2rem] border-2 border-slate-100 bg-white hover:border-slate-200 transition-all flex flex-col justify-between shadow-sm hover:shadow-md">
                      <div>
                        <div className="flex items-center justify-between mb-4">
                          <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{t('monthly')}</p>
                          <div className="h-8 w-8 rounded-lg bg-slate-50 flex items-center justify-center border border-slate-100 group-hover:bg-slate-100 transition-colors">
                            <Rocket className="h-4 w-4 text-slate-400 group-hover:text-slate-600" />
                          </div>
                        </div>
                        <div className="flex items-baseline gap-1 mb-1">
                          <span className="text-3xl font-black text-slate-900 tracking-tight">39 kr</span>
                          <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">/{locale === 'nb' ? 'mnd' : 'mo'}</span>
                        </div>
                        <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest mb-6 bg-blue-50/50 py-1 px-2 rounded-lg inline-block">
                          {t('pro.trial')}
                        </p>
                      </div>
                      <Link href="/settings">
                        <Button variant="outline" className="w-full text-[10px] font-black h-11 uppercase tracking-widest border-slate-200 hover:bg-slate-900 hover:text-white transition-all rounded-xl">
                          {t('startTrial')}
                        </Button>
                      </Link>
                    ...
                    {/* Yearly Option */}
                    <div className="group relative p-6 rounded-[2rem] border-2 border-blue-100 bg-gradient-to-br from-blue-50/50 to-white hover:border-blue-300 transition-all flex flex-col justify-between shadow-sm hover:shadow-xl hover:-translate-y-1">
                      <div className="absolute -top-3 right-4 px-3 py-1 bg-blue-600 text-white text-[9px] font-black uppercase tracking-[0.2em] rounded-full shadow-lg shadow-blue-200 z-10">
                        {locale === 'nb' ? 'BESTE OMSETNING' : 'MOST POPULAR'}
                      </div>

                      <div>
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-2">
                            <p className="text-[10px] font-black uppercase text-blue-600 tracking-widest">{t('yearly')}</p>
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-[8px] font-black rounded uppercase">
                              {locale === 'nb' ? 'Spar 25%' : 'Save 25%'}
                            </span>
                          </div>
                          <div className="h-8 w-8 rounded-lg bg-blue-600 flex items-center justify-center border border-blue-700 shadow-sm transition-transform group-hover:scale-110">
                            <Sparkles className="h-4 w-4 text-white" />
                          </div>
                        </div>

                        <div className="flex items-baseline gap-1 mb-1">
                          <span className="text-3xl font-black text-slate-900 tracking-tight">349 kr</span>
                          <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">/{locale === 'nb' ? 'år' : 'yr'}</span>
                        </div>
                        <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest mb-6 bg-blue-50/50 py-1 px-2 rounded-lg inline-block">
                          {t('pro.trial')}
                        </p>
                      ...
                      <Link href="/settings">
                        <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-black h-11 uppercase tracking-widest transition-all rounded-xl shadow-lg shadow-blue-100">
                          {t('startTrial')}
                        </Button>
                      </Link>
